<?php
  ...
  $mainBody = '';
  $count = 0;
  $query = "SELECT id, userID, bookID, tag, issueDate FROM `issue` WHERE userID = '".mysqli_real_escape_string($conn, $row['id'])."'";
      ...
      if(mysqli_num_rows($result)>0){
          while($row1 = mysqli_fetch_assoc($result)){
            if($row1['tag'] == "1"){
             ...
             <td><form method="POST"><button type="submit" class="btn btn-outline-primary" name="returnBook" value="2">Return</button>
      ...
      <div class="col-md-2 border border-top-0 py-2" id="dash" style="text-align: center;">
        ...
        <ul class="nav  nav pills flex-column">
          ...
          <li class="nav-item bg-primary rounded">
            <a class="nav-link disabled text-light">My Books</a>
          </li>
          <li class="nav-item">
