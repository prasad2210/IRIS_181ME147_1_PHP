<?php
  ...
  if (array_key_exists("id", $_SESSION) ) {
   ...
   $mainBody = '';
   $count = 0;
   $query = "SELECT id, userID, bookID, tag FROM `issue`";
   ...
   if($result){
       if(mysqli_num_rows($result)>0){
           while($row1 = mysqli_fetch_assoc($result)){
             if($row1['tag'] == "0"){
              ...
              if($row2['quantity'] < 1){
                $approveButton = 'disabled>Not Available';
                $rejectButton = 'disabled>Not Available';
              }
              $mainBody .= '<tr>
              ...
              <td>'.$row2["isbn"].'</td>
              <td>'.$row2["quantity"].'</td>
              <td><form method="POST"><button type="submit" class="btn btn-outline-primary" name="issueAction" value="1"'.$approveButton.'</button>
                  <input type="hidden" name="id" value="'.$row1['id'].'"> 
                  <button type="submit" class="btn btn-outline-primary" name="issueAction" value="-1"'.$rejectButton.'</button></form>
              </td>
            </tr>';
          }
        }
    ...
    $error = '';
    if(array_key_exists("issueAction", $_POST)){
      $query = "SELECT `tag`, `bookID` FROM `issue` WHERE id = '".mysqli_real_escape_string($conn, $_POST['id'])."'";
      $result = mysqli_query($conn, $query);
      $row1 = mysqli_fetch_array($result);
      if($row1['tag'] == 0){
        if($_POST['issueAction'] == "1"){
          $issueDate = date('Y-m-d');
        }
        else{
          $issueDate = "-";
        }
        $query = "UPDATE `issue` SET tag = '".mysqli_real_escape_string($conn, $_POST['issueAction'])."', issueDate = '".date('Y-m-d')."'
          ...
          if($_POST['issueAction'] == '1'){
            ...
            if($row2['quantity']){
              ...
              if(mysqli_query($conn, $query1)) {
                $error = '<div class="alert alert-success text-center col-md-12" role="alert">Request Approved<button type="button" class="close" data-dismiss="alert">x</button> </div>';
                header("Refresh:0");
              }
              else{
                $error = '<div class="alert alert-danger text-center col-md-12" role="alert">There is some error processing this request<button type="button" class="close" data-dismiss="alert">x</button> </div>';
                header("Refresh:0");
              }
            }
          }
          else{
            $error = '<div class="alert alert-warning text-center col-md-12" role="alert">Request Rejected<button type="button" class="close" data-dismiss="alert">x</button> </div>';
          }
        }
        else{
          $error = '<div class="alert alert-danger text-center col-md-12" role="alert">There is some error processing this request<button type="button" class="close" data-dismiss="alert">x</button> </div>';
        ...
        $error = '<div class="alert alert-danger text-center col-md-12" role="alert">Cannot process a request twice.<button type="button" class="close" data-dismiss="alert">x</button> </div>';
      }
    }
      ...
      <div class="col-md-7 py-2">
        ...
        <nav aria-label="breadcrumb">
          <ol class="breadcrumb remalign">
            ...
            <li class="breadcrumb-item">issues-pending</li>
          </ol>
        </nav>
      </div>
      ...
      <ul class="nav flex-column">
          ...
          <li class="nav-item ">
            ...
            <!-- <div id="submenu" class="collapse"> -->
              <ul>
                <li><a class="disabled " >Pending</a></li>
